...
// Synced with sovereignty-ai-studio. Rotate with: node gen_passphrases.js
const PP_HASHES = {
...
function coquiSpeak(text, wsId, voice, speed) {
  ...
  return new Promise(resolve => {
    ...
    const params = new URLSearchParams({ text: safe, speaker_id: '', style_wav: '', language_id: '' });
    if (voice) params.set('speaker_id', voice);
    const url = new URL(COQUI_URL + '/api/tts?' + params.toString());
    const opts = { hostname: url.hostname, port: url.port, path: url.pathname + url.search, method: 'GET', timeout: 30000 };
    const req = http.request(opts, res => {
      if (res.statusCode !== 200) { res.resume(); resolve({ type:'coqui_done', fallback:true, status:res.statusCode }); return; }
      const chunks = [];
    ...
    req.on('error', () => { coquiReady = false; resolve({ type:'coqui_done', fallback:true }); });
    req.on('timeout', () => { req.destroy(); resolve({ type:'coqui_done', fallback:true, error:'timeout' }); });
    req.end();
  });
}
...
function runDiagnostic(ws) {
  const status = {
    type: 'diagnostic_report', ts: new Date().toISOString(),
    server: { uptime: Math.floor(process.uptime()), memory: process.memoryUsage(), pid: process.pid, nodeVersion: process.version },
    tts: { piperReady, coquiReady, coquiURL: COQUI_URL, queueLength: ttsQueue.length, processing: ttsProcessing },
    websocket: { clients: clients.size },
    ai: { claude: !!process.env.ANTHROPIC_API_KEY, gpt: !!process.env.OPENAI_API_KEY, grok: !!process.env.XAI_API_KEY },
    errors: [],
  };
  if (!piperReady && !coquiReady) status.errors.push({code:'NO_TTS',message:'Neither Piper nor Coqui TTS is available'});
  if (!process.env.ANTHROPIC_API_KEY && !process.env.OPENAI_API_KEY && !process.env.XAI_API_KEY)
    status.errors.push({code:'NO_AI_KEYS',message:'No AI API keys configured'});
...
const httpServer = http.createServer(async (req, res) => {
  ...
  if (url === '/health' || url === '/api/health') {
    json(200, { status:'ok', ts:new Date().toISOString(), piper:piperReady, coqui:coquiReady, uptime:Math.floor(process.uptime()) });
    return;
  }
  ...
  if (req.method === 'POST') {
    ...
    if (url === '/api/auth/login') { const r = await doLogin(body, ip); json(r.s, r.d); return; }
    if (url === '/api/auth/verify-panel') { const r = doVerifyPanel(body, ip); json(r.s, r.d); return; }
    ...
    if (url === '/' || url === '/dashboard' || url === '/FullDashboard.html') {
      ...
      if (fs.existsSync(dashboardPath)) {
        try {
          ...
          return;
        } catch(e) { json(500, {detail: 'Error reading dashboard: ' + e.message}); return; }
      } else { json(404, {detail: 'Dashboard file not found'}); return; }
    }
...
wss.on('connection', (ws, req) => {
  ...
  ws.on('message', async raw => {
    ...
    if (type === 'ping') { ws.send(JSON.stringify({type:'pong',ts:Date.now(),piper:piperReady,coqui:coquiReady,wsId,uptime:Math.floor(process.uptime())})); return; }
    if (type === 'piper_speak' || type === 'speak') { if (!msg.text) { ws.send(JSON.stringify({type:'error',code:'NO_TEXT'})); return; } ws.send(JSON.stringify(await enqueueTTS(msg.text, wsId, 'piper', '', msg.speed||1.0))); return; }
    if (type === 'coqui_speak') { if (!msg.text) { ws.send(JSON.stringify({type:'error',code:'NO_TEXT'})); return; } ws.send(JSON.stringify(await enqueueTTS(msg.text, wsId, 'coqui', msg.voice||'', msg.speed||1.0))); return; }
    if (type === 'tts_speak') { if (!msg.text) { ws.send(JSON.stringify({type:'error',code:'NO_TEXT'})); return; } ws.send(JSON.stringify(await enqueueTTS(msg.text, wsId, msg.engine||'auto', msg.voice||'', msg.speed||1.0))); return; }
    if (type === 'tts_status') { ws.send(JSON.stringify({type:'tts_status',piper:piperReady,coqui:coquiReady,queueLength:ttsQueue.length,processing:ttsProcessing})); return; }
    if (type === 'diagnostic') { runDiagnostic(ws); return; }
    if (type === 'ddg_search') { const r = await ddgSearch(msg.query||''); ws.send(JSON.stringify({type:'ddg_result',...r})); return; }
    if (type === 'ai_query') { aiProxy(msg.model||'claude', msg.text||'', msg.system||'', msg.apiKey||'', msg.max_tokens, (err, response) => { ws.send(JSON.stringify({type:'ai_response', text:response, model:msg.model||'claude'})); }); return; }
    ...
    if (type === 'memory_get') { ws.send(JSON.stringify({type:'memory_result',cards:memStore[msg.key||wsId]||[]})); return; }
...
httpServer.listen(PORT_UNIFIED, '127.0.0.1', () => {
  ...
  process.stdout.write('|  Piper TTS '+(piperReady?'OK Ready    ':'-- Not found  ')+'                       |\n');
  process.stdout.write('|  Coqui TTS '+(coquiReady?'OK Ready    ':'-- Not found  ')+'                       |\n');
