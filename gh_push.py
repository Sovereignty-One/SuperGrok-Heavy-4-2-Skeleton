...
def main():
    ...
    if repo_status != 200:
        ...
        print('   Check that the repo exists and your token has access.')
        sys.exit(1)
    print(f'  OK: {repo_data.get("full_name")} -- {repo_data.get("visibility", "?")} -- '
          f'{repo_data.get("default_branch")} branch')

    print(f'\n[2/4] Getting {BRANCH} branch SHA...')
    branch_sha = get_branch_sha()
    print(f'  OK: HEAD {branch_sha[:12]}...')
